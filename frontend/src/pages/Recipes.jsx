...
const Recipes = () => {
  ...
  const handleDeleteRecipe = async (e, id) => {
    e.stopPropagation();
    if (window.confirm('Excluir esta Receita/Pré-preparo desmoronará todas as Receitas que dependem dela! Deseja prosseguir de fato?')) {
      try {
        ...
        <div className="xl:col-span-3 space-y-6">
          {!isCreatingNew && !selectedRecipe ? (
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-800 xl:min-h-[700px] overflow-hidden flex flex-col animate-in fade-in transition duration-300">
              ...
              {/* Header do Sistema */}
              <div className="bg-gradient-to-r from-brand-600 to-brand-800 dark:from-brand-800 dark:to-brand-900 p-6 text-white flex justify-between items-center gap-4">
                ...
                <div>
                  <label className="text-brand-100 text-[10px] uppercase tracking-wider font-semibold mb-1 block text-center">Rendimento Bruto</label>
                  <div className="flex items-center gap-2">
                    <input type="number" step="0.1" name="yield_quantity" value={calcForm.yield_quantity} onChange={handleChange} min="0.1" className="bg-white/10 border border-white/20 rounded-lg px-3 py-2.5 text-white w-24 focus:outline-none focus:ring-2 focus:ring-white/50 text-center font-bold text-lg"/>
                    <select name="yield_unit" value={calcForm.yield_unit} onChange={handleChange} className="bg-white/10 border border-white/20 rounded-lg px-3 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-white/50 text-sm font-bold uppercase cursor-pointer">
                  ...
                  <section>
                    ...
                    <div className="bg-gray-50 dark:bg-zinc-800/30 rounded-xl p-4 border border-gray-100 dark:border-zinc-800/50 mb-4">
                          ...
                          {recipeIngredients.map(ing => (
                              <div key={ing.relation_id} className="flex justify-between items-center bg-white dark:bg-zinc-900 p-2.5 rounded-lg border border-gray-100 dark:border-zinc-800 shadow-sm group">
                                ...
                                <div className="flex items-center gap-3">
                                  <button onClick={() => handleRemoveItem(ing.relation_id)} className="text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 dark:bg-zinc-800 p-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Trash2 size={14}/>
                                  </button>
                                </div>
                              </div>
                      ...
                      <form onSubmit={handAddItem} className="mt-4 flex gap-2 pt-4 border-t border-gray-200 dark:border-zinc-700">
                        <select required className="input-field flex-1 text-sm py-2 px-2" value={itemForm.component_id} onChange={(e) => setItemForm({...itemForm, component_id: e.target.value})}>
                        ...
                        <input type="number" step="0.001" required className="input-field w-16 text-sm py-2 px-1 text-center" placeholder="Qtd." value={itemForm.quantity} onChange={(e) => setItemForm({...itemForm, quantity: e.target.value})}/>
                        <select className="input-field w-16 text-sm py-2 px-1 bg-gray-100 dark:bg-zinc-800 font-bold cursor-pointer" value={itemForm.unit} onChange={(e) => setItemForm({...itemForm, unit: e.target.value})}>
                        ...
                        <button type="submit" title="Vincular à Ficha" className="bg-brand-100 text-brand-700 hover:bg-brand-200 dark:bg-brand-900 dark:text-brand-300 dark:hover:bg-brand-800 px-3 rounded-lg font-bold shadow-sm transition"><Plus size={16}/></button>
                      </form>
                    </div>
                  </section>
                  ...
                  <div>
                    ...
                    <div className="mt-8 mb-4">
                       ...
                       <div className="flex gap-2">
                          <input type="number" step="0.1" name="profit_margin" value={calcForm.profit_margin} onChange={handleChange} className="input-field text-lg font-bold w-32 shadow-inner" placeholder="Ex: 3"/>
                          <div className="text-[10px] text-gray-400 self-center leading-tight">
